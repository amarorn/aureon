...
export class AppointmentController {
  ...
  async outlookStatus(@TenantId() tenantId: string) {
    const diagnostics = await this.outlookCalendar.getDiagnostics(tenantId);
    return {
      connected: diagnostics.ready,
      ...diagnostics,
    };
  ...
  @Post('outlook-calendar/sync')
  syncToOutlook(@TenantId() tenantId: string) {
    return this.service.syncAllToOutlook(tenantId);
  }

  @Post('outlook-calendar/import')
  importFromOutlook(
    @TenantId() tenantId: string,
    @Query('startDate') startDate?: string,
    @Query('endDate') endDate?: string,
    ...
    const end =
      endDate ?? new Date(Date.now() + 90 * 24 * 60 * 60 * 1000).toISOString();
    return this.service.importFromOutlook(tenantId, start, end);
  }
